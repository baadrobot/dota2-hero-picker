...
class simpleMysqli extends simpleDB
{
    ...
    public function __construct($connectionSettingsArray)
    {
        ...
        //kainax
        $this->db_resource->options(MYSQLI_OPT_CONNECT_TIMEOUT, 60);
    ...
    {
        //kainax: //$lastInsertId = $dbClass->queryInfo['insert_id']; //last insert id //last_insert_id
    ...
    protected function prepareQuery(&$arguments)
    {
        ...
        $sprintfArg[] = str_replace("%", "%%", $arguments[0]); // Kainax added str_replace() to prevent errors on a text with percent sign (%)
        foreach ($arguments as $pos => $var) {
            if (is_array($var)) {
                ...
                $sprintfArg[] = implode(',', $replaceWith); //str_replace("%", "%%",
            }
        }
